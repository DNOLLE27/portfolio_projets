...
{
	$contenuHeader[$numCellEntete++]='[C] Formule' ; // Affichage du nom de colonne : Formule
    $contenuHeader[$numCellEntete++]='[C] Nombre de repas' ;
}
...
foreach($lesRepasPris as $ligne)
{
	...
	{
		$laFormuleAffiche = $ligne['libelleFormule'] ;
		$contenuTableau[$numCellDetail++]= ''.$laFormuleAffiche ;
		if ($_SESSION['typeRepas'] == 3)
		{
			$contenuTableau[$numCellDetail++]='[R] '.$ligne['libelleFormule'] ; 
			$cumulNbRepas += $ligne['nbRepas'] ;
		}
	}
	//$laDateAffiche = $ligne['dateMenu'] ;
